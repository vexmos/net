...
public class BungeeConfig {
    ...
    public BungeeConfig(String name) {
        File dataFolder = Bungee.get().getDataFolder();
    ...
    public void reloadConfig() {
        try {
            ...
            InputStream inputStream = Bungee.get().getResourceAsStream(this.file.getName());
            if (inputStream != null) {
            ...
            Bungee.get().getLogger().info("Não foi possível reiniciar a config " + file.getName());
        }
    }
    ...
    public void saveConfig() {
        try {
            ...
            Bungee.get().getLogger().info("Não foi possível salvar a config " + file.getName());
        }
    }
    ...
    public void saveDefault() {
        setDefaults(ConfigurationProvider.getProvider(YamlConfiguration.class).load(Bungee.get().getResourceAsStream(this.file.getName())));
        saveConfig();
    }
    ...
    public void saveResource(String resourcePath, boolean b) {
        InputStream in = Bungee.get().getResourceAsStream(resourcePath);
        if (in != null) {
            File outFile = new File(Bungee.get().getDataFolder(), resourcePath);
            ...
            try {
                ...
                Bungee.get().getLogger().info("Erro ao salvar o recurso " + resourcePath + ": " + e.getMessage());
            } finally {
                try {
                    in.close();
                    if (out != null)
                    ...
                    Bungee.get().getLogger().info("Recurso " + resourcePath + " não encontrado");
                }
            }
        }
    }
    ...
    public void saveDefaultConfig() {
        File configFile = new File(Bungee.get().getDataFolder(), this.file.getName());
