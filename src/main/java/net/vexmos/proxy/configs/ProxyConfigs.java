...
public class ProxyConfigs {
     ...
     */
    public ProxyConfigs(String name) {
        File dataFolder = Proxy.get().getDataFolder();
    ...
    public void reloadConfig() {
        try {
            ...
            InputStream inputStream = Proxy.get().getResourceAsStream(this.file.getName());
            if (inputStream != null) {
            ...
            Proxy.get().getLogger().info("Não foi possível reiniciar a config " + file.getName());
        }
    }
    ...
    public void saveConfig() {
        try {
            ...
            Proxy.get().getLogger().info("Não foi possível salvar a config " + file.getName());
        }
    }
    ...
    public void saveDefault() {
        setDefaults(ConfigurationProvider.getProvider(YamlConfiguration.class).load(Proxy.get().getResourceAsStream(this.file.getName())));
        saveConfig();
    }
    ...
    public void saveResource(String resourcePath, boolean b) {
        InputStream in = Proxy.get().getResourceAsStream(resourcePath);
        if (in != null) {
            File outFile = new File(Proxy.get().getDataFolder(), resourcePath);
            ...
            try {
                ...
                Proxy.get().getLogger().info("Erro ao salvar o recurso " + resourcePath + ": " + e.getMessage());
            } finally {
                try {
                    in.close();
                    if (out != null)
                    ...
                    Proxy.get().getLogger().info("Recurso " + resourcePath + " não encontrado");
                }
            }
        }
    }
    ...
    public void saveDefaultConfig() {
        File configFile = new File(Proxy.get().getDataFolder(), this.file.getName());
